...
const regionErrorSudoku: number[][] = [
  [5, 3, 4, 6, 7, 8, 9, 1, 2],
  [6, 7, 2, 1, 9, 5, 3, 4, 8],
  [1, 9, 8, 3, 4, 2, 5, 6, 7],
  [8, 5, 9, 7, 6, 1, 4, 2, 3],
...
function checkSudoku(sudoku: number[][]): string {
  let message = "VALID";
  let rows = new Map<number, number[]>();
  let columns = new Map<number, number[]>();
  let regionsSample: number[][][] = [];
...
function isSetValid(set: number[]): boolean {
